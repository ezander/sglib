...
ind=ismember(1:n_germ, dim_index);
% Reduced multiindex set
I_new=I(:, ind);
% Reduced polynomial system
sys_new=sys(ind);


%% Calculate response values at the fixed  germs
...
u_fixed=gpcbasis_evaluate(V_fixed, val);

%% Output
V_u_red={sys_new, I_new};
if isempty(u_i_alpha)
    u_i_alpha=ones(size(I,1), 1);
end
u_i_beta=binfun(@times, u_i_alpha, u_fixed');
end