function xi = gpc_sample(V, n, varargin)
...
%
%   XI = GPC_SAMPLE(V, N, 'mode', 'qmc') creates the random samples via
%   transformed uniform [0,1] Quasi Monte Carlo random numbers generated by
%   the standard Halton sequence.
%
%   XI = GPC_SAMPLE(V, N, 'mode', 'lhs') creates the random samples via
%   transformed uniform [0,1] Latin hypercube samples.
%
%   XI = GPC_SAMPLE(V, N, 'rand_func', @my_rand_func) creates the random
...
switch(mode)
    ...
    case 'lhs'
        U = lhs_uniform(n, m);
    otherwise
        error('sglib:gpc_sample', 'Unknown paramter value "%s" for option "mode"', mode);
end
