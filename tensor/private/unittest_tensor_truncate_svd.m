...
%
% See also TENSOR_TRUNCATE_SVD, MUNIT_RUN_TESTSUITE 
...
[U,sigma,k]=tensor_truncate_svd( T, G, 0, inf, true, 2 );
assert_equals( sigma, sv, 'sigma' );
assert_equals( U{1}*U{2}', T{1}*T{2}', 'reproduce' );

% check that k_max is honored over eps
[U,sigma,k]=tensor_truncate_svd( T, G, 0, 1, true, 2 );
assert_equals( sigma, sv, 'k1sigma' );
assert_equals( tensor_rank(U), 1, 'rank1' );
[U,sigma,k]=tensor_truncate_svd( T, G, 0, 3, true, 2 );
assert_equals( sigma, sv, 'k3sigma' );
assert_equals( tensor_rank(U), 3, 'rank3' );
...
releps=abseps/sv(1);
[U,sigma,k]=tensor_truncate_svd( T, G, abseps, inf, false, inf );
assert_equals( k, k_ex, 'k' );
