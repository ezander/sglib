...
% CHECK_RANGE Check whether input argument is in range.
%   OK=CHECK_RANGE( X, LO, HI, VARNAME, MFILENAME ) checks whether input
%   parameter X is in the range [LO,HI] (inclusive bounds). VARNAME and
%   MFILENAME are strings used for printing sensible error messages.
%
...
if x<lo || x>hi
    ...
    error([mfilename ':range'], 'input argument "%s" not in range [%g,%g]: %g', varname, lo, hi, x )
end
